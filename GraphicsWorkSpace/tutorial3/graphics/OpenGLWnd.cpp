...
BOOL COpenGLWnd::CreateRGBPalette(HDC hDC)
{
    ...
    if (!pPal) 
    {
        AfxMessageBox("Out of memory for pallet", MB_OK | MB_ICONSTOP);
        return FALSE;
    }
...
void COpenGLWnd::OnSaveImage()
{
   ...
   //
	...
	static char BASED_CODE szFilter[] = "Image Files (*.bmp)|*.bmp|All Files (*.*)|*.*||";

	CFileDialog dlg(FALSE, ".bmp", NULL, 0, szFilter, NULL);
	if(dlg.DoModal() != IDOK)
   {
      delete buffer[0];
      delete buffer;
   ...
   if(!file)
   {
      AfxMessageBox("Unable to write output file file", IDOK);
      delete buffer[0];
      delete buffer;
...
bool COpenGLWnd::ObtainPixels(GLbyte **&p_pixels)
{
	...
	if (!bResult)
	{
      AfxMessageBox("Unable to select OpenGL rendering context, could not obtain pixel data.");
      p_pixels = NULL;
      return false;
	}
