	...
	public static void main(String[] args) {
		...
		for (int t = 1; t <= T; t++) {
			...
			while (q-- > 0) {
				...
				/*System.out.println("Minimum for range:"+l+","+r);
				int left=l;
				int right=l+(1<<k)-1;
				System.out.println("Minimum in the interval "+left+","+right+" is "+table[l][k]);
				left=r-(1<<k)+1;
				right=r-(1<<k)+1+(1<<k)-1;
				System.out.println("Minimum in the interval "+left+","+right+" is "+table[r-(1<<k)+1][k]);*/
	...
	private static void buildSparseTable(int n) {
		...
		for (int j = 1; (1 << j) <= n; j++) {
			for (int i = 0; (i + (1 << j)) <= n; i++) {
				...
				//int left=i;
				//int right=i+(1<<j)-1;
				//System.out.println("Minimum in the interval "+left+","+right+" is "+table[i][j]);
			}
		}
